...
const char * const BITCOIN_CONF_FILENAME = "slavecoin.conf";
const char * const BITCOIN_PID_FILENAME = "slavecoin.pid";
...
#else
    const char* pszModule = "slavecoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Slavecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Slavecoin";
#else
    // Unix
    return pathRet / ".slavecoin";
#endif
#endif
}
